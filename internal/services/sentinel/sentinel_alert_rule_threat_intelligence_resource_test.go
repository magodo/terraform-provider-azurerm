...
func (r SentinelAlertRuleThreatIntelligenceResource) Exists(ctx context.Context, client *clients.Client, state *pluginsdk.InstanceState) (*bool, error) {
	...
	if !ok {
		return nil, fmt.Errorf("the Alert Rule %q is not a ThreatIntelligence Alert Rule", id)
	}
...
resource "azurerm_sentinel_alert_rule_threat_intelligence" "test2" {
  name                       = "acctest-SentinelAlertRule-ThreatIntelligence-2-%d"
  log_analytics_workspace_id = azurerm_log_analytics_solution.test.workspace_resource_id
