...
func resourceSentinelAlertRuleThreatIntelligence() *pluginsdk.Resource {
	return &pluginsdk.Resource{
		Create: resourceSentinelAlertRuleThreatIntelligenceCreateUpdate,
		Read:   resourceSentinelAlertRuleThreatIntelligenceRead,
		Update: resourceSentinelAlertRuleThreatIntelligenceCreateUpdate,
		Delete: resourceSentinelAlertRuleThreatIntelligenceDelete,

		Importer: pluginsdk.ImporterValidatingResourceIdThen(func(id string) error {
			_, err := parse.AlertRuleID(id)
			return err
		}, importSentinelAlertRule(securityinsight.AlertRuleKindThreatIntelligence)),
...
func resourceSentinelAlertRuleThreatIntelligenceCreateUpdate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if d.IsNewResource() {
		...
		if err != nil {
			if !utils.ResponseWasNotFound(resp.Response) {
				return fmt.Errorf("checking for existing Sentinel Alert Rule ThreatIntelligence %q: %+v", id, err)
			}
		}

		id := alertRuleID(resp.Value)
		if id != nil && *id != "" {
			return tf.ImportAsExistsError("azurerm_sentinel_alert_rule_threat_intelligence", *id)
		}
	}

	params := securityinsight.ThreatIntelligenceAlertRule{
	...
	if !d.IsNewResource() {
		resp, err := client.Get(ctx, workspaceID.ResourceGroup, OperationalInsightsResourceProvider, workspaceID.WorkspaceName, name)
		if err != nil {
			return fmt.Errorf("retrieving Sentinel Alert Rule ThreatIntelligence %q: %+v", id, err)
		}

		if err := assertAlertRuleKind(resp.Value, securityinsight.AlertRuleKindThreatIntelligence); err != nil {
			return fmt.Errorf("asserting alert rule of %q: %+v", id, err)
		}
		params.Etag = resp.Value.(securityinsight.ThreatIntelligenceAlertRule).Etag
	}
	...
	if _, err := client.CreateOrUpdate(ctx, workspaceID.ResourceGroup, OperationalInsightsResourceProvider, workspaceID.WorkspaceName, name, params); err != nil {
		return fmt.Errorf("creating Sentinel Alert Rule ThreatIntelligence %q: %+v", id, err)
	}

	d.SetId(id.ID())

	return resourceSentinelAlertRuleThreatIntelligenceRead(d, meta)
}

func resourceSentinelAlertRuleThreatIntelligenceRead(d *pluginsdk.ResourceData, meta interface{}) error {
	client := meta.(*clients.Client).Sentinel.AlertRulesClient
	...
	if err != nil {
		if utils.ResponseWasNotFound(resp.Response) {
			log.Printf("[DEBUG] Sentinel Alert Rule ThreatIntelligence %q was not found - removing from state!", id)
			d.SetId("")
			return nil
		}
...
func resourceSentinelAlertRuleThreatIntelligenceDelete(d *pluginsdk.ResourceData, meta interface{}) error {
	client := meta.(*clients.Client).Sentinel.AlertRulesClient
	...
	if _, err := client.Delete(ctx, id.ResourceGroup, OperationalInsightsResourceProvider, id.WorkspaceName, id.Name); err != nil {
		return fmt.Errorf("deleting Sentinel Alert Rule ThreatIntelligence %q: %+v", id, err)
	}
